...
from .config import Config, git_commit, weight_filename
from .runner import PROJECT_ROOT

# Software, hardware and input checksums of the environment that produced the paper's runs.
PAPER_ENVIRONMENT = PROJECT_ROOT / "experiments" / "paper_environment.json"

# Tolerances for `compare`, on the per-benchmark means of the quality tables. They sit above
# the largest run-to-run range of those means in the paper's own data (HOTA 0.41, IDF1 0.57,
# MOTA 0.13, IDSW 3, Frag 4), so a faithful re-run passes and a behaviour change does not.
...
def file_sha256(path: str | Path, text: bool = False) -> str:
    # Text files are hashed with CRLF normalized to LF: Python writes CRLF on Windows, so a GT
    # prepared there and one prepared on Linux must hash the same.
...
def check_software(cfg: Config, manifest: dict) -> list[tuple[str, str, str]]:
    rows: list[tuple[str, str, str]] = []

    exp_py, cur_py = manifest.get("python", ""), platform.python_version()
    same_minor = cur_py.split(".")[:2] == exp_py.split(".")[:2]
    rows.append(("PASS" if same_minor else "WARN", "python", f"{cur_py} (paper {exp_py})"))

    for dist, exp in manifest.get("library_versions", {}).items():
    ...
    if str(cfg.device).lower() != "cpu":
        paper_gpu = manifest.get("hardware", {}).get("gpu", "?")
        try:
            import torch
        ...
        except Exception as exc:  # noqa: BLE001 - report any import problem as a failure
            rows.append(("FAIL", "cuda", f"torch cannot be imported: {exc}"))

    te = manifest.get("trackeval", {})
    ...
    if not (root / "scripts" / "run_mot_challenge.py").exists():
        ...
        commit, exp = git_commit(root), te.get("commit", "")
        rows.append(("PASS" if commit == exp else "WARN", "trackeval commit",
                     f"{commit[:12]} (paper {exp[:12]})"))

    # Probe from the clone, as evaluate.call_trackeval runs it: `trackeval` then imports from
    # the checkout whether or not it was pip-installed into the venv.
    ...
    try:
        ...
        # TrackEval prints optional-dependency notices (e.g. BURST without pycocotools) on
        # import, so only the last line is the probe's answer.
...
def check_files(cfg: Config, manifest: dict) -> list[tuple[str, str, str]]:
    rows: list[tuple[str, str, str]] = []

    # Ultralytics loads (and downloads) detector weights from the working directory.
    ...
    for name, expected in manifest.get("weights_sha256", {}).items():
    ...
    gt_hashes = manifest.get("prepared_gt_sha256", {})
...
def compare_results(reference_dir: str | Path, candidate_dir: str | Path,
                    tol: dict | None = None) -> tuple[bool, list[str]]:
    tol = tol or TABLE_TOL
    ref = pd.read_csv(Path(reference_dir) / "all_results.csv")
    new = pd.read_csv(Path(candidate_dir) / "all_results.csv")
    # Compare like with like: a partial run (fewer detectors, sequences or benchmarks) is checked
    # against the reference rows of the same configurations and sequences only.
    ...
    for trk in sorted(set(ref["tracker"])):
        ...
        # The headline result: the best HOTA of each benchmark.
